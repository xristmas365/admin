...
<?php if(!$node->isNewRecord || !empty($parentKey)): ?>
    ...
    $name = $node->getBreadcrumbs($depth, $glue, $activeCss, 'New Catalog');
    ...
    $nameField = $showNameAttribute ? $form->field($node, $nameAttribute)->textInput(array_merge($inputOpts, ['placeholder' => 'Catalog Name']))->label(false) : '';
    ?>
    <?php
    /**
     ...
     */
    ?>
    <div class="kv-detail-heading bg-light">
        <?php if(empty($inputOpts['disabled']) || ($isAdmin && $showFormButtons)): ?>
            <div class="float-right pull-right">
                <?= Html::submitButton('<i class="fas fa-cloud-upload-alt"></i> Save', ['class' => 'btn btn-white', 'title' => $submitTitle]) ?>
            </div>
        <?php endif; ?>
